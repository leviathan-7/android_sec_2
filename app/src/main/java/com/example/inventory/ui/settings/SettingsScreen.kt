...
private fun SettingsDetailsBody(
    ...
    Column(
        ...
        var checkedisDefolt by remember { mutableStateOf(viewModel.isDefolt()) }
        Row(){
            Text(text = "Использовать дефолтное количество ")
            Switch(checked = checkedisDefolt, onCheckedChange = {
                viewModel.switchDefolt()
                checkedisDefolt = it
            })
        }
        var def by remember { mutableStateOf(viewModel.defolt().toString()) }
        if (checkedisDefolt){
            Row(){
                Text(text = "Дефолтное количество товара ")
                OutlinedTextField(
                    value = def,
                    onValueChange = {
                        viewModel.newDefolt(it)
                        def = it
                    },
